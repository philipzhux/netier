...
void Epoll::delIOContext(IOContext *c) { del(c->getFd()); }
...
void Epoll::add(Socket const &socket) { add(socket, EPOLLIN | EPOLLET); }

void Epoll::add(int fd) { add(fd, EPOLLIN | EPOLLET); }

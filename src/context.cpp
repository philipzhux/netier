...
void Context::flushWriteBuffer()
{
    ...
    while (__wbuffer->size() > 0)
    {
        ...
        {
            __state = CLOSED;
            printf("[Context::flushWriteBuffer] Read EOF, client fd %d disconnected\n", __socket_fd);
            this->destroy();
            break;
        }
        else
        {
            __state = INVALID;
            printf("Context::flushWriteBuffer] Unknown error on fd=%d, socket closed and context destroyed\n", __socket_fd);
            this->destroy();
            break;
        }
    }
...
void Context::handleReadableEvent()
{
    ...
    while (1)
    {
        ...
        else if (bytes_read == 0)
        {
            __buffer->shrink(Context::read_buf_size);
            __state = CLOSED;
            printf("[Context::handleReadableEvent] Read EOF, client fd %d disconnected\n", fd);
            this->destroy();
            return;
        }
        else
        {
            __buffer->shrink(Context::read_buf_size);
            __state = INVALID;
            printf("[Context::handleReadableEvent] Unknown error on fd=%d, socket closed and context destroyed\n", fd);
            this->destroy();
            return;
        }
    }
...
ER Context::syncWrite(const void *buf, size_t size)
{
    ...
    while (remaining > 0)
    {
        ...
        {
            __state = INVALID;
            printf("[Context::syncWrite] Unknown error on fd=%d, socket closed and context destroyed\n", __socket_fd);
            this->destroy();
            return ER::SOCKET_ERROR;
        }
    }

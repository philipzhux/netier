...
void Server::serve() {
  ...
  std::cout << "Listening on " << __addr.getAddressString() << " ..."
            << std::endl;
...
const Context &Server::contextCreator(int fd, Address addr) {
  std::unique_ptr<Context> newContext = std::make_unique<Context>(
      fd, std::move(addr), schedule(fd), __onConn,
      std::bind(&Server::contextDestroyer, this, std::placeholders::_1));
  if (__onRecv)
...
void Server::contextDestroyer(int fd) { __contextes.erase(fd); }
